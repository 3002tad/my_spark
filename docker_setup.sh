...
sudo docker run -d -p 27017:27017 --network mynetwork --name mymongodb mongo:4.4
sleep 3

# Chạy container unica-full với biến môi trường Mongo_HOST
sudo docker run -e Mongo_HOST=mymongodb --network mynetwork --name unica-full 3002tad/unica_full_data
sleep 3

# Chạy container PostgreSQL với các thông số user, password, và database
sudo docker run --name mypostgres --network mynetwork -e POSTGRES_USER=postgres -e POSTGRES_PASSWORD=12345 -e POSTGRES_DB=unica_db -p 5432:5432 -d postgres
sleep 3

# Tạo bảng instructor trong database unica_db
sudo docker exec -it mypostgres psql -U postgres -d unica_db -c "CREATE TABLE instructor (instructor_id SERIAL PRIMARY KEY, instructor_name VARCHAR(255) UNIQUE);"
sleep 3
...
sudo docker exec -it mypostgres psql -U postgres -d unica_db -c "CREATE TABLE course_tag (tag_id SERIAL PRIMARY KEY, tag_name VARCHAR(255) UNIQUE);"
sleep 3
